...
void* Connection::ConnectionThreadWrapper(void* p_Connection)
{
    Connection::ConnectionThread(p_Connection);
    return nullptr;
}
...
void Connection::ConnectionThread(void* p_Connection)
{
    ...
    ssize_t s_Ret = -1;
    while ((s_Ret = recv(s_Connection->GetSocket(), &s_IncomingMessageSize, sizeof(s_IncomingMessageSize), 0)) > 0)
    {
        ...
        s_Ret = recv(s_Connection->GetSocket(), s_IncomingMessageData, s_IncomingMessageSize, 0);
