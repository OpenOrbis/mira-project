...
int FakeSelfManager::OnSceSblAuthMgrIsLoadable2(struct self_context* p_Context, struct self_auth_info* p_OldAuthInfo, int32_t p_PathId, struct self_auth_info* p_NewAuthInfo)
{
    return 0;
}
...
int FakeSelfManager::OnSceSblAuthMgrVerifyHeader(struct self_context* p_Context)
{
    return 0;
}
...
int FakeSelfManager::SceSblAuthMgrSmLoadSelfBlock(uint32_t p_ServiceId, void* p_Request, void* p_Response)
{
	...
	SelfContext* ctx = *(SelfContext**)(frame - 0x1C8);
#endif
    ...
    bool s_IsUnsigned = ctx->format == SelfFormat::Elf;
    if (!s_IsUnsigned)
        return SceSblServiceMailbox(p_ServiceId, p_Request, p_Response);
    ...
    vm_offset_t s_SegmentDataCpuVa = 0;
}
